...
if (isset($_POST["submit"])) {
    ...
    if (!empty($_POST["email"]) and !empty($_POST["password"]) ) {
        $sql="SELECT * FROM `hostel_information` WHERE `Email`='$mail' AND `Password`='$psw' ";
        $result=mysqli_query($link,$sql);
        $row=mysqli_fetch_assoc($result);
        if($row>0){
            $_SESSION["email"]=$mail;
            echo "<script>
            document.location='hostel.php';
            </script>";
        }
        else{
            $sql1="SELECT * FROM `hostel_information` WHERE `Email`='$mail'";
            ...
            if($row1>0){                
                ...
                echo "<h1>Email ID not regestered.</h1>";
                }
        }
    }
    else{
...
<form action="<?php echo $_SERVER["PHP_SELF"];?> " method="post" >
    <h1>Login as a Hostel</h1>
      ...
      <a class="signup-link" href="register_hostel.php">Not registered?want to register your hostel</a>
